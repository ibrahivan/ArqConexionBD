...
public class ImplConexion implements InterfazConexion {
	...
	public Connection generaConexion() {
		...
		if(!parametrosConexion[2].isEmpty()) { //Se controla que los parámetros de conexión se completen
			try {
				...
				System.out.println(esValida ? "[INFORMACIÓN-ImplConexion-generaConexion] Conexión a PostgreSQL válida" : "[ERROR-ConexionImplementacion-generaConexion] Conexión a PostgreSQL no válida");
	            return conexion;
	            
			} catch (ClassNotFoundException cnfe) {
				System.out.println("[ERROR-ImplConexion-generaConexion] Error en registro driver PostgreSQL: " + cnfe);
				return conexion = null;
			} catch (SQLException jsqle) {
				System.out.println("[ERROR-ImplConexion-generaConexion] Error en conexión a PostgreSQL (" + parametrosConexion[0] + "): " + jsqle);
				return conexion = null;
			}
			
		}else {
			System.out.println("[ERROR-ImplConexion-generaConexion] Los parametros de conexion no se han establecido correctamente");	
			return conexion;
		}
	...
	private String[] configuracionConexion() {
		...
		try {
			...
			propiedadesConexion.load(new FileInputStream(new File("C:${symbol_escape}${symbol_escape}Users${symbol_escape}${symbol_escape}Puesto12${symbol_escape}${symbol_escape}source${symbol_escape}${symbol_escape}repos${symbol_escape}${symbol_escape}Conexion${package}Java${symbol_escape}${symbol_escape}conexion${package}${symbol_escape}${symbol_escape}src${symbol_escape}${symbol_escape}util${symbol_escape}${symbol_escape}datos.properties")));
			...
			System.out.println("[ERROR-ImplConexion-configuracionConexion] - Error al acceder al fichero propiedades de conexion.");
			return null;
		}
